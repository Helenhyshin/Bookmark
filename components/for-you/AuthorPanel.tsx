  ...
  return (
    <div className="flex flex-wrap gap-4">
      {favoriteAuthors.map((author) => {
        ...
        return (
          <div key={author} className="w-[240px] h-[200px] shrink-0 bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex flex-col overflow-hidden">
            <div className="flex items-center gap-2 mb-2 shrink-0">
              <div className="w-8 h-8 rounded-full bg-[#D4AF37]/20 flex items-center justify-center shrink-0">
                <Star size={14} className="text-[#D4AF37] fill-[#D4AF37]" />
              </div>
              <h3 className="font-serif font-semibold text-sm leading-tight truncate min-w-0">{author}</h3>
            </div>
            ...
            {unread.length > 0 && (
              <div className="mt-2 space-y-1.5 flex-1 min-h-0 overflow-y-auto">
